...
public interface PaymentModeRepository extends JpaRepository<PaymentMode,Integer> {
    ...
    @Query(value="select expense_type,description,value,payment_mode,user_expense.expense_date from user_expense JOIN payment_mode ON user_expense.user_id=payment_mode.user_id where payment_mode = 'UPI';")
    List<>
}
